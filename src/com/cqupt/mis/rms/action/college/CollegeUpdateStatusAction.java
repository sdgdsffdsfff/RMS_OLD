...
public class CollegeUpdateStatusAction extends ActionSupport{
	...
	public String execute() throws Exception{
		...
		String approvedUserId = (String) ActionContext.getContext().getSession().get("userId");
		boolean result = false;
		if(returnReason==null){
			//审批通过
			if("1".equals(status)) status = "2";
			if("5".equals(status)) status = "6";
			Float verify_amounts = 0.0f;
			
			
			result = collegeManagerService.updateInfoByIdNew(id, modelName, status, idName,approvedUserId,verify_amounts);
		}else {
			//拒绝审批
			if("1".equals(status)) status = "3";
			if("5".equals(status)) status = "7";
			if("2".equals(status)) status = "3";
			if("6".equals(status)) status = "7";
			
			result = collegeManagerService.updateRejectInfoById(id, modelName, status, idName,approvedUserId,returnReason);
