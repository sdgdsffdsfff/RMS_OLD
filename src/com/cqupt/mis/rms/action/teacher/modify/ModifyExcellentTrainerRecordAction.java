...
public class ModifyExcellentTrainerRecordAction extends ActionSupport implements ServletContextAware {
	...
	public String execute() throws Exception {
		...
		excellentTrainerRecord.setId(recordId);
		excellentTrainerRecord.setSubmitUser(user);
		excellentTrainerRecord.setName(recordName);
		...
		List<ExcellentRecordAward> excellentRecordAwardsLists = new ArrayList<ExcellentRecordAward>();
		try {
			//上传旁证材料
			if(upload != null){
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++) {
					if(!"".equals(memberName[j]) && memberName[j] != null) {
						...
						excellentRecordAwardsLists.add(excellentRecordAward);
					}
				}
			}
		} catch(Exception e) {
		...
		boolean result1 = dynamicDataRecordDao.updateRecord(excellentTrainerRecord);
		boolean result2 = researchInfoService.modifyProofs(proofs);
		boolean result3 = researchInfoService.modifyResearchMemberInfo(recordId, "ExcellentRecordAward", "excellentTrainerRecord", "id", 27, excellentRecordAwardsLists);//TODO
