...
public class ModifyCourseContributeAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		CourseContribute courseContribute = (CourseContribute)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(courseId, "CourseContribute", "courseId");
		...
		try {
			...
			boolean result1 = researchInfoService.modifyResearchInfo(courseContribute);
			boolean result2 = researchInfoService.modifyResearchMemberInfo(courseId, "CourseContributeMember", "courseContribute", "courseId", 14, courseContributeMembers);
			boolean result3 = researchInfoService.modifyProofs(proofs);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("课程建设信息修改成功");
				confirm.setUrl("viewCourseContribute.action");
				confirm.setRetName("个人课程建设信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("课程建设信息修改失败");
				confirm.setUrl("viewCourseContribute.action");
				confirm.setRetName("个人课程建设信息页面");
			}
