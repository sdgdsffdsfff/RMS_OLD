...
public class ModifyLearningEvaluationRecordAction extends ActionSupport implements ServletContextAware {
	...
	public String execute() throws Exception {
		...
		learningEvaluationRecord.setId(recordId);
		learningEvaluationRecord.setSubmitUser(user);
		learningEvaluationRecord.setName(recordName);
		...
		List<LearningRecordAward> learningRecordAwards = new ArrayList<LearningRecordAward>();
		try {
			//上传旁证材料
			if(upload != null){
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++) {
					if(!"".equals(memberName[j]) && memberName[j] != null) {
						...
						learningRecordAwards.add(learningRecordAward);
					}
				}
			}
		} catch(Exception e) {
		...
		boolean result1 = dynamicDataRecordDao.updateRecord(learningEvaluationRecord);
		boolean result2 = researchInfoService.modifyProofs(proofs);
		boolean result3 = researchInfoService.modifyResearchMemberInfo(recordId, "LearningRecordAward", "learningEvaluationRecord", "id", 31, learningRecordAwards);//TODO
		Confirm confirm = new Confirm();
		if(result1 && result2 && result3){
			confirm.setIsSuccess("right");
			confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(7)+"修改成功");
			...
			confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(7)+"修改失败");
			confirm.setUrl("viewLearningEvaluationRecords.action");
			confirm.setRetName("管理个人"+DynamicDataFieldUtils.getInfoNameByClassNum(7)+"页面");
		}
