...
/**
*<p>Title:处理教师用户提交学生获奖信息的Action</p>
*<p>Description:接收用户提交的信息并处理</p>
*@author LvHai
*@version 1.0
**/
@SuppressWarnings("serial")
public class ModifyStudentAwardsAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		List<StudentInstructor> studentInstructors = new ArrayList<StudentInstructor>();
		String targetDirectory = servletContext.getRealPath(GenerateUtils.generateSavePath());//获得路径
		
		StudentAwards studentAwards = (StudentAwards)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(awardsId, "StudentAwards", "awardsId");
		...
		try {
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++){
					if(!"".equals(memberName[j]) && memberName[j] != null){
						...
						studentInstructor.setMemberName(memberName[j]);
						if(!"".equals(remarksMem[j]) && remarksMem[j] != null){
							studentInstructor.setRemarks(remarksMem[j]);
						}
			...
			boolean result1 = researchInfoService.modifyResearchInfo(studentAwards);
			boolean result2 = researchInfoService.modifyProofs(proofs);
			boolean result3 = researchInfoService.modifyResearchMemberInfo(awardsId, "StudentInstructor", "studentAwards", "awardsId", 16, studentInstructors);
