...
public class ModifyScienceBookAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		ScienceBook scienceBook = (ScienceBook)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(bookId, "ScienceBook", "bookId");
		...
		try {
			...
			boolean result1 = researchInfoService.modifyResearchInfo(scienceBook);
			boolean result2 = researchInfoService.modifyProofs(proofs);
			boolean result3 = researchInfoService.modifyResearchMemberInfo(bookId, "ScienceBookAuthor", "scienceBook", "bookId", 3, scienceBookAuthors);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("理科著作信息修改成功");
				confirm.setUrl("viewScienceBook.action");
				confirm.setRetName("个人理科著作信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("理科著作信息修改失败");
				confirm.setUrl("viewScienceBook.action");
				confirm.setRetName("个人理科著作信息页面");
			}
