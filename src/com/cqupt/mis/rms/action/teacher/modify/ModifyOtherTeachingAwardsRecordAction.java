...
public class ModifyOtherTeachingAwardsRecordAction extends ActionSupport implements ServletContextAware {
	...
	public String execute() throws Exception {
		...
		otherTeachingAwardsRecord.setId(recordId);
		otherTeachingAwardsRecord.setSubmitUser(user);
		otherTeachingAwardsRecord.setName(recordName);
		...
		List<OtherTeachingRecordAward> otherTeachingRecordAwards = new ArrayList<OtherTeachingRecordAward>();
		try {
			//上传旁证材料
			if(upload != null){
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++) {
					if(!"".equals(memberName[j]) && memberName[j] != null) {
						...
						otherTeachingRecordAwards.add(otherTeachingRecordAward);
					}
				}
			}
		} catch(Exception e) {
		...
		boolean result1 = dynamicDataRecordDao.updateRecord(otherTeachingAwardsRecord);
