...
public class ModifyQualityProjectRecordAction extends ActionSupport implements ServletContextAware {
	...
	public String execute() throws Exception {
		...
		qualityProjectRecord.setId(recordId);
		qualityProjectRecord.setSubmitUser(user);
		qualityProjectRecord.setName(recordName);
		...
		List<QualityRecordAward> qualityRecordAwards = new ArrayList<QualityRecordAward>();
		try {
			//上传旁证材料
			if(upload != null){
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++) {
					if(!"".equals(memberName[j]) && memberName[j] != null) {
						...
						qualityRecordAwards.add(qualityRecordAward);
					}
				}
			}
		} catch(Exception e) {
		...
		boolean result1 = dynamicDataRecordDao.updateRecord(qualityProjectRecord);
