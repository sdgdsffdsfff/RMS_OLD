...
/**
*<p>Title:处理教师用户提交科技交流信息的Action</p>
*<p>Description:接收用户提交的信息并处理</p>
*@author LvHai
*@version 1.0
**/
@SuppressWarnings("serial")
public class ModifyScienceTechExchangeAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		List<ScienceTechAttendPerson> scienceTechAttendPersons = new ArrayList<ScienceTechAttendPerson>();
		String targetDirectory = servletContext.getRealPath(GenerateUtils.generateSavePath());//获得路径
		
		ScienceTechExchange scienceTechExchange = (ScienceTechExchange)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(techExchangeId, "ScienceTechExchange", "techExchangeId");
		...
		try {
			...
			if(upload != null){
				for (int i = 0; i < upload.length; i++){
			        ...
			        proof.setInfoApprovedId(techExchangeId);
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++){
					if(!"".equals(memberName[j]) && memberName[j] != null){
						...
						scienceTechAttendPerson.setMemberName(memberName[j]);
						if(!"".equals(remarksMem[j]) && remarksMem[j] != null){
							scienceTechAttendPerson.setRemarks(remarksMem[j]);
						}
			...
			boolean result1 = researchInfoService.modifyResearchInfo(scienceTechExchange);
			boolean result2 = researchInfoService.modifyProofs(proofs);
			boolean result3 = researchInfoService.modifyResearchMemberInfo(techExchangeId, "ScienceTechAttendPerson", "scienceTechExchange", "techExchangeId", 6, scienceTechAttendPersons);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("理科科技交流信息修改成功");
				confirm.setUrl("viewScienceTechExchange.action");
				confirm.setRetName("个人理科科技交流信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("理科科技交流信息修改失败");
				confirm.setUrl("viewScienceTechExchange.action");
				confirm.setRetName("个人理科科技交流信息页面");
			}
