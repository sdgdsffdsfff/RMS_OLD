...
public class ModifyScienceIpRightsAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		ScienceIpRights scienceIpRights = (ScienceIpRights)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(rightsId, "ScienceIpRights", "rightsId");
		...
		try {
			...
			if(upload != null){
				for (int i = 0; i < upload.length; i++){
			        ...
			        proof.setInfoApprovedId(rightsId);
			...
			boolean result1 = researchInfoService.modifyResearchInfo(scienceIpRights);
			boolean result2 = researchInfoService.modifyProofs(proofs);
			boolean result3 = researchInfoService.modifyResearchMemberInfo(rightsId, "ScienceInventors", "scienceIpRights", "rightsId", 5, scienceInventors);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("理科知识产权信息修改成功");
				confirm.setUrl("viewScienceIpRights.action");
				confirm.setRetName("个人理科知识产权信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("理科知识产权信息修改失败");
				confirm.setUrl("viewScienceIpRights.action");
				confirm.setRetName("个人理科知识产权信息页面");
			}
