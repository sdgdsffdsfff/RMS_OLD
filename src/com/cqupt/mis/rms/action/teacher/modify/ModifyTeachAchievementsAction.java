...
/**
*<p>Title:处理教师用户提交教学成果奖信息的Action</p>
*<p>Description:接收用户提交的信息并处理</p>
*@author LvHai
*@version 1.0
**/
@SuppressWarnings("serial")
public class ModifyTeachAchievementsAction extends ActionSupport implements ServletContextAware {
	//注入接口
	...
	public String execute() throws Exception {
		...
		List<TeachersAwards> teachersAwards = new ArrayList<TeachersAwards>();
		String targetDirectory = servletContext.getRealPath(GenerateUtils.generateSavePath());//获得路径
		
		TeachAchievements teachAchievements = (TeachAchievements)researchInfoDao.findResearchInfoByIdAndModelNameAndFactor(achievementsId, "TeachAchievements", "achievementsId");
		...
		try {
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++){
					if(!"".equals(memberName[j]) && memberName[j] != null){
						TeachersAwards teachersAward = new TeachersAwards();
						teachersAward.setTeachAchievements(teachAchievements);
			...
			boolean result1 = researchInfoService.modifyResearchInfo(teachAchievements);
			boolean result2 = researchInfoService.modifyProofs(proofs);
			boolean result3 = researchInfoService.modifyResearchMemberInfo(achievementsId, "TeachersAwards", "teachAchievements", "achievementsId", 15, teachersAwards);
