...
public class SubmitOtherTeachingAwardsRecordAction extends ActionSupport implements ServletContextAware{
		...
		private OtherTeachingAwardsRecord otherTeachingAwardsRecord;
		...
		public String execute() throws Exception {
			 ...
			 * 构建一条记录对象
			 */
			OtherTeachingAwardsRecord  otherTeachingAwardsRecord = new OtherTeachingAwardsRecord();
			otherTeachingAwardsRecord.setId(id);
			otherTeachingAwardsRecord.setName(projectName);
			otherTeachingAwardsRecord.setSubmitUser(user);
			otherTeachingAwardsRecord.setStatus(status);
			List<OtherTeachingAwardsField> fieldsFromDataBase = searchDao.SearchObjectsByFactor("com.cqupt.mis.rms.model.OtherTeachingAwardsField", "isDelete", 0);
			Set<OtherTeachingAwardsData> fields = new HashSet<OtherTeachingAwardsData>();
			//将前台已填写的字段封装成TeachersAwardsData对象
			
			for(OtherTeachingAwardsField f : fieldsFromDataBase) {
				String value = (String) request.getParameter(f.getName());
				if(value==null || "".equals(value)) {
					continue;
				} else {
				
					OtherTeachingAwardsData tempData = new OtherTeachingAwardsData();
					tempData.setRecord(otherTeachingAwardsRecord);
					tempData.setField(f);
					tempData.setValue(value);
					fields.add(tempData);
				}
			}
			otherTeachingAwardsRecord.setFields(fields);
			...
			List<OtherTeachingRecordAward> otherTeachingRecordAwardList = new ArrayList<OtherTeachingRecordAward>();
				...
				if(memberName != null){
					for(int j = 0; j < memberName.length; j++){
						if(!"".equals(memberName[j]) && memberName[j] != null){
							OtherTeachingRecordAward otherTeachingRecordAward = new OtherTeachingRecordAward();
							otherTeachingRecordAward.setOtherTeachingAwardsRecord(otherTeachingAwardsRecord);
							otherTeachingRecordAward.setMemberName(memberName[j]);
							if(!"".equals(remarksMem[j]) && remarksMem[j] != null){
								otherTeachingRecordAward.setRemarks(remarksMem[j]);
							}
							otherTeachingRecordAwardList.add(otherTeachingRecordAward);
						}
					}
				}
				
				boolean result1 = dynamicDataRecordDao.addRecord(otherTeachingAwardsRecord);
				boolean result2 = submitInfoAndProofsService.submitResearchMemberInfo(34, otherTeachingRecordAwardList);
				boolean result3 = submitInfoAndProofsService.submitProofs(proofs);
				
				Confirm confirm = new Confirm();
				if(result1 && result2 && result3){
					confirm.setIsSuccess("right");
					confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(9)+"添加成功");
					...
					confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(9)+"添加失败");
					confirm.setUrl("viewOtherTeachingAwardsRecords.action");
					confirm.setRetName("管理个人"+DynamicDataFieldUtils.getInfoNameByClassNum(9)+"页面");
				}
