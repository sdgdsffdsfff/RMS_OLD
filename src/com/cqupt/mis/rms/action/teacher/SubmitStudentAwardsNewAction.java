...
public class SubmitStudentAwardsNewAction extends ActionSupport implements ServletContextAware {
	//注入服务层接口
	...
	public String execute() throws Exception {
		...
		List<StudentInstructorNew> studentInstructors = new ArrayList<StudentInstructorNew>();
		String id = GenerateUtils.getID();//生成ID
		String targetDirectory = context.getRealPath(GenerateUtils.generateSavePath());//获得路径
		StudentAwardsNew studentAwardsNew = new StudentAwardsNew();
		studentAwardsNew.setAwardsId(id);
		...
		{
			studentAwardsNew.setCollegeAward(0f);
		}
		studentAwardsNew.setSubmitUser(user);
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++){
					if(!"".equals(memberName[j]) && memberName[j] != null){
						...
						studentInstructors.add(studentInstructorNew);
					}
				}
			}
			
			boolean result1 = submitInfoAndProofsService.submitInfo(studentAwardsNew);
			boolean result2 = submitInfoAndProofsService.submitProofs(proofs);
			boolean result3 = submitInfoAndProofsService.submitResearchMemberInfo(22, studentInstructors);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("指导学生参赛获奖 信息添加成功");
				confirm.setUrl("viewStudentAwardsNew.action");
				confirm.setRetName("个人指导学生参赛获奖 信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("指导学生参赛获奖 信息添加失败");
				confirm.setUrl("viewStudentAwardsNew.action");
				confirm.setRetName("个人指导学生参赛获奖 信息页面");
			}
