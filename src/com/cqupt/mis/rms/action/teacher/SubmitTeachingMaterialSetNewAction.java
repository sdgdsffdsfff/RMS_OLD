...
public class SubmitTeachingMaterialSetNewAction extends ActionSupport implements ServletContextAware {
	//注入服务层接口
	...
	public String execute() throws Exception {
		...
		List<TeachingMaterialEditorNew> teachingMaterialEditorsNew = new ArrayList<TeachingMaterialEditorNew>();
		String id = GenerateUtils.getID();//生成ID
		String targetDirectory = context.getRealPath(GenerateUtils.generateSavePath());//获得路径
		TeachingMaterialSetNew teachingMaterialSetNew =new TeachingMaterialSetNew();
		teachingMaterialSetNew.setTeachingMaterialId(id);
		teachingMaterialSetNew.setNumberProject(numberProject);
		teachingMaterialSetNew.setResultsPostedStatus(resultsPostedStatus);
		teachingMaterialSetNew.setSetClass(setClass);
		teachingMaterialSetNew.setSetTime(setTime);
		if(""!=wordsNumbers)
		{
			teachingMaterialSetNew.setWordsNumbers(Float.parseFloat(wordsNumbers));
			
		}else{
			teachingMaterialSetNew.setWordsNumbers(0f);

		}
		if(""!=collegeAward)
		{
			teachingMaterialSetNew.setCollegeAward(Float.parseFloat(collegeAward));

		}else{
			teachingMaterialSetNew.setCollegeAward(0f);

		}
		teachingMaterialSetNew.setRemarks(remarks);
		
		teachingMaterialSetNew.setTeachingMaterialName(teachingMaterialName);
		teachingMaterialSetNew.setSubmitUser(user);
		teachingMaterialSetNew.setStatus(status);
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++){
					if(!"".equals(memberName[j]) && memberName[j] != null){
						...
						teachingMaterialEditor.setTeachingMaterialSetNew(teachingMaterialSetNew);
						...
						teachingMaterialEditorsNew.add(teachingMaterialEditor);
					}
				}
			}
			
			boolean result1 = submitInfoAndProofsService.submitInfo(teachingMaterialSetNew);
			boolean result2 = submitInfoAndProofsService.submitResearchMemberInfo(24, teachingMaterialEditorsNew);
			boolean result3 = submitInfoAndProofsService.submitProofs(proofs);
			Confirm confirm = new Confirm();
			if(result1 && result2 && result3){
				confirm.setIsSuccess("right");
				confirm.setMessage("教材出版信息添加成功");
				confirm.setUrl("viewTeachingMaterialSetNew.action");
				confirm.setRetName("个人教材出版信息页面");
			}else{
				confirm.setIsSuccess("error");
				confirm.setMessage("教材出版信息添加失败");
				confirm.setUrl("viewTeachingMaterialSetNew.action");
				confirm.setRetName("个人教材出版信息页面");
			}
