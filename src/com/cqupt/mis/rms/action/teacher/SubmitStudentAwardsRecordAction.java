...
public class SubmitStudentAwardsRecordAction extends ActionSupport implements ServletContextAware {
	...
	public String execute() throws Exception {
		...
		List<StudentRecordInstructor> studentRecordInstructors = new ArrayList<StudentRecordInstructor>();
		try {
			//上传旁证材料
			if(upload != null){
			...
			if(memberName != null){
				for(int j = 0; j < memberName.length; j++) {
					if(!"".equals(memberName[j]) && memberName[j] != null) {
						StudentRecordInstructor studentRecordInstructor = new StudentRecordInstructor();
						studentRecordInstructor.setStudentAwardsRecord(studentAwardsRecord);
						studentRecordInstructor.setMemberName(memberName[j]);
						if(!"".equals(remarksMem[j]) && remarksMem[j] != null) {
							studentRecordInstructor.setRemarks(remarksMem[j]);
						}
						studentRecordInstructors.add(studentRecordInstructor);
					}
				}
			}
		} catch(Exception e) {
		...
		if(result1 && result2 && result3){
			...
			confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(5)+"添加成功");
			confirm.setUrl("viewStudentAwardsRecords.action");
			confirm.setRetName("管理个人"+DynamicDataFieldUtils.getInfoNameByClassNum(5)+"页面");
		} else {
			confirm.setIsSuccess("error");
			confirm.setMessage(DynamicDataFieldUtils.getInfoNameByClassNum(5)+"添加失败");
			confirm.setUrl("viewStudentAwardsRecords.action");
			confirm.setRetName("管理个人"+DynamicDataFieldUtils.getInfoNameByClassNum(5)+"页面");
		}
