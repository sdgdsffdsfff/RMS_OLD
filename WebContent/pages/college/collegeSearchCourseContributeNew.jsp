...
<head>
    ...
    <script src="js/search/SearchCourseContributeNew.js" type="text/javascript"></script>
...
<div class="l-loading" style="display:block" id="pageloading"></div> 
 ...
 <form id="form1" action="searchCollegeCourseContributeNew.action" method="post">
<div id="hippo">
	<ul class="list">
		<li class="til">
		...
		<li class="item">
			...
			<select class="logical_item" name="stringName1" id="stringName1">
				<option>请选择</option>
				<option value="courseId">本科教学工程信息编号</option>
				<option value="classContribute">项目级别</option>
				<option value="typeContribute">项目类别</option>
				<option value="courseName">项目名称</option>
				<option value="timeContribute">项目来源</option>
		...
		<li class="item">
			...
			<select class="logical_item" name="dateName" id="dateName">
				<option>请选择</option>
				<option value="checkTime">立项时间</option>
   ...
   <c:if test="${not empty courseContributeNewInfos}">
     	 <c:forEach items="${courseContributeNewInfos}" var="courseContributeNewInfo">
     	 	<script type="text/javascript">
				var row = {courseId: "${courseContributeNewInfo.model.courseId}",
						   classContribute: "${courseContributeNewInfo.model.classContribute}", 
						   typeContribute: "${courseContributeNewInfo.model.typeContribute}", 
						   timeContribute: "${courseContributeNewInfo.model.timeContribute}", 
						   courseName:"${courseContributeNewInfo.model.courseName}",
						   checkTime:"${courseContributeNewInfo.model.checkTime}",
						   endTime:"${courseContributeNewInfo.model.endTime}",
						   collegeAward:"${courseContributeNewInfo.model.collegeAward}",
						   submitUser:"${courseContributeNewInfo.model.submitUser.userName}",
						   approvedUser:"${courseContributeNewInfo.model.approvedUser.userName}",
						   Status: "${courseContributeNewInfo.model.status}",
						   type: "${type}"
						};
				rows.push(row);
    		 </script>
